...
- **[MIT report on AI and Education](https://aiandeducation.mit.edu/report/)** (MIT Ad Hoc Committee on AI Use in Teaching, Learning, and Research Training, August 2026) — earmarked for the future **Teacher Resources** section rather than for card content. Worth being precise about its scope: it's about *generative AI in higher education at MIT*, written for faculty and administrators, and it contains no K-12 classroom guidance. What it does offer this project is teacher-facing framing — its guiding principles ("put humanity first," "augmentation not automation," be humble / be bold) and its emphasis on preserving human connection and intentional course design line up closely with why this game is deliberately unplugged. Use it to support the *why* in teacher materials; keep using the AI4K12 report and the AI Pedagogy Project for what goes on the cards.
...
1. ~~Playtest the new Audit math~~ — three rounds of playtesting so far:
   ...
   - **Round 10 — Quick Play, simulated at scale** (tax-from-3rd-System rate): all 10 two-suit decks, each made legal at 20+ cards (every card of both suits, topped up with second copies of cheap Commons), 600 games per pairing — 27,000 games, both sides using the same simple bot strategy.
     - **Pacing is healthy:** median game 7 rounds (14 turns), with 80% of games between 6 and 8 rounds. Short, consistent, fits a class period. The tax rate holds up.
     - **First player wins 57%.** A moderate edge, similar to many TCGs; worth watching, but no fix needed yet.
     - **Green is the strongest suit and Black the weakest.** Deck win rates range from Green+Red 74% down to Black+White 34%; all four Green decks finish top 4, and all Black decks except Green+Black finish bottom 3.
     - **Why Black is weak — efficiency, not fragility:** Black Systems cost 3.0 Data on average but score about the same Tokens as Green's 2-Data Systems, so Black gets **0.70 Tokens per Data spent vs. Green 1.27 and Red 1.47**. Black's high Capability only helps in Audits, and Audits don't score. Black Systems are Audited out about as often as everyone else's (47%).
   ...
   - **Round 11 — Quick Play on the 60-card pool** (5 logged games plus a 22,500-game round-robin, legal 20+ card decks): game length held at 7 rounds, but three problems showed up and all three were fixed.
     - **Model Card + a cheap attacker was a loop.** Model Card's save was *once per turn*, so a 1-Trust, high-Capability System (Deepfake especially) could Audit every turn and be revived every turn. One logged game had a single Deepfake knock out four Systems and never die. It appeared in 9% of games, with 3+ saves in 4%. **Fixed: Model Card is now once per game.**
