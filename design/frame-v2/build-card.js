...
const PALETTES = {
  ...
  ethics: { art: 'ethics', edge: '#e2c06a', outerA: '#efe5cb', outerB: '#d3c292', body1: '#f8f2e2', body2: '#e1d3ae', head: '#3a3224',
    ...
    discussInk: '#2b2418', discussShadow: 'none', typeGlass: 'rgba(255,250,236,.72)' },
...
// Gold coin, one per Token: milled rim, embossed hexagon "token" mark, top-left shine.
const coin = id => `<svg class="coin" viewBox="0 0 44 44" width="44" height="44" aria-hidden="true"><defs>
  <radialGradient id="cg${id}" cx="36%" cy="30%" r="72%"><stop offset="0" stop-color="#fff5bd"/><stop offset=".45" stop-color="#f3c34f"/><stop offset="1" stop-color="#a66a14"/></radialGradient></defs>
  <circle cx="22" cy="22" r="20.5" fill="url(#cg${id})" stroke="#6f440b" stroke-width="2"/>
  <circle cx="22" cy="22" r="15.5" fill="none" stroke="#b37c20" stroke-width="1.6" stroke-dasharray="2 2.2"/>
  <polygon points="28.5,18.25 22,14.5 15.5,18.25 15.5,25.75 22,29.5 28.5,25.75" fill="none" stroke="#7a4b0d" stroke-width="2.2" stroke-linejoin="round"/>
  <circle cx="22" cy="22" r="2.4" fill="#7a4b0d"/>
  <path d="M10.5 15 A13.5 13.5 0 0 1 21 8.5" fill="none" stroke="#fffbe3" stroke-width="2.2" stroke-linecap="round" opacity=".85"/></svg>`;
...
function cardHtml(c, suit = c.suit) {
  ...
  const vars = ['edge','outerA','outerB','body1','body2','head','panel1','panel2','label','bracket','circuit','statA','statB','statBorder','ledgeA','ledgeB','discussBg','discussBorder','discussBracket','discussTag','discussInk','discussShadow','ink','typeInk','headInk','statInk','typeGlass'];
  ...
  return `<div class="v2${full ? ' full' : ''}${isSystem ? '' : ' nostats'}" style="${style}">
  <div class="v2-body">
...
const CSS = `
  ...
  /* art window */
  .v2-art{ position:absolute; left:26px; right:26px; top:118px; height:318px; background:var(--art) center/cover;
  ...
  .v2-ledge{ position:absolute; left:240px; right:240px; top:414px; height:30px; z-index:2;
    background:linear-gradient(180deg,var(--ledgeA),var(--ledgeB)); clip-path:polygon(8% 0,92% 0,100% 100%,0 100%); }
  .v2-stats{ position:absolute; left:0; right:0; top:328px; display:flex; justify-content:center; gap:22px; z-index:5; }
  ...
  .v2-kind{ position:absolute; top:388px; left:50%; transform:translateX(-50%); z-index:5; display:flex; align-items:center; gap:12px;
  ...
  /* type bar */
  .v2-type{ position:absolute; left:34px; right:30px; top:458px; height:46px; display:flex; align-items:center; gap:18px; color:var(--typeInk); }
  ...
  .brk::before{ content:""; position:absolute; inset:-9px; pointer-events:none;
    ...
    mask:
      ...
      linear-gradient(#000 0 0) left bottom / 58px 58px no-repeat,
      linear-gradient(#000 0 0) right bottom / 58px 58px no-repeat; }
  .v2-text{ --r:26px; left:40px; right:40px; top:518px; height:236px; padding:20px 32px; border-radius:var(--r);
...
document.fonts.ready.then(function(){
  document.querySelectorAll('.v2').forEach(function(card){
    ...
    var TOP = tp.offsetTop, MAX = 236, MIN = 132, GAP = 22;
    function layout(f){
...
function writeCard(name, suitOverride) {
  ...
  const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') + '-' + suit;
...
<body><div class="stage">${cardHtml(c, suit)}</div><script>${FIT}</script></body></html>`;
  fs.writeFileSync(path.join(OUT, `proof-${slug}.html`), html);
  return `design/frame-v2/proof-${slug}.html`;
}

fs.mkdirSync(OUT, { recursive: true });
if (process.argv[2] === '--all') {
  const names = [...src.matchAll(/<h2[^>]*>([^<]+)<\/h2>/g)].map(m => m[1]);
  names.forEach(n => writeCard(n));
  console.log(`wrote ${names.length} cards to design/frame-v2/`);
} else {
  console.log('wrote', writeCard(process.argv[2] || 'The Human Trainer', process.argv[3]));
}
